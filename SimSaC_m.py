...
from SimSaC.feature_backbones.VGG_features import VGGPyramid
from SimSaC.feature_backbones.ResNet_features import ResNetPyramid
from SimSaC.our_models.mod import CMDTop,ConvDecoder #, deconvPAC
from SimSaC.our_models.mod import OpticalFlowEstimatorNoDenseConnection, OpticalFlowEstimator, FeatureL2Norm, \
    CorrelationVolume, deconv, conv, predict_flow, unnormalise_and_convert_mapping_to_flow, warp
from SimSaC.our_models.consensus_network_modules import MutualMatching, NeighConsensus, FeatureCorrelation
os.environ['PYTHON_EGG_CACHE'] = 'tmp/' # a writable directory 
from SimSaC.correlation import correlation # the custom cost volume layer
import numpy as np
...
class SimSaC_Model(nn.Module):
    ...
    def __init__(self, evaluation, div=1.0, iterative_refinement=False,
        ...
        if self.use_pac: from SimSaC.our_models.mod import deconvPAC
        self.div=div
    ...
    def forward_sigle_ref(self, im_target, im_source, im_target_256, im_source_256,disable_flow=None):
        ...
        corr3 = correlation.FunctionCorrelation(tensorFirst=c13.cuda(), tensorSecond=warp3.cuda())
        ...
        if self.decoder_inputs == 'corr_flow_feat':
            corr3 = torch.cat((corr3.cpu(), up_flow4), 1)
        elif self.decoder_inputs == 'corr':
            corr3 = corr3
        elif self.decoder_inputs == 'corr_flow':
        ...
        corr2 = correlation.FunctionCorrelation(tensorFirst=c12.cuda(), tensorSecond=warp2.cuda())
        ...
        if self.decoder_inputs == 'corr_flow_feat':
            corr2 = torch.cat((corr2.cpu(), up_flow3), 1)
        elif self.decoder_inputs == 'corr':
            corr2 = corr2
        elif self.decoder_inputs == 'corr_flow':
        ...
        corr1 = correlation.FunctionCorrelation(tensorFirst=c11.cuda(), tensorSecond=warp1.cuda())
        ...
        if self.decoder_inputs == 'corr_flow_feat':
            corr1 = torch.cat((corr1.cpu(), up_flow2, up_feat2), 1)
        elif self.decoder_inputs == 'corr':
            corr1 = corr1
