...
import torch
import torch.nn as nn
def conv1x1_bam(in_channels, out_channels, stride=1):
    ''' 1x1 convolution '''
    return nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)

def conv3x3_bam(in_channels, out_channels, stride=1, padding=1, dilation=1):
    ''' 3x3 convolution '''
    return nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=padding, dilation=dilation, bias=False)

def conv7x7_bam(in_channels, out_channels, stride=1, padding=3, dilation=1):
    ''' 7x7 convolution '''
...
class BAM(nn.Module):
    def __init__(self, in_channel, reduction_ratio=2, dilation=2):
        ...
        self.conv1 = conv1x1_bam(in_channel, self.hid_channel)
        self.bn1_2d = nn.BatchNorm2d(self.hid_channel)
        self.conv2 = conv3x3_bam(self.hid_channel, self.hid_channel, stride=1, padding=self.dilation, dilation=self.dilation)
        self.bn2_2d = nn.BatchNorm2d(self.hid_channel)
        self.conv3 = conv3x3_bam(self.hid_channel, self.hid_channel, stride=1, padding=self.dilation, dilation=self.dilation)
        self.bn3_2d = nn.BatchNorm2d(self.hid_channel)
        self.conv4 = conv1x1_bam(self.hid_channel, 1)
...
class MGAM(nn.Module):
    def __init__(self, in_channel= 1024):
        ...
        self.bam = BAM(in_channel=in_channel //2)
...
class LSGCANet_BAM_SiameseNet(nn.Module):
    def __init__(self):
        super(LSGCANet_BAM_SiameseNet, self).__init__()
